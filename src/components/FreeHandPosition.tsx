...
export const FreeHandPosition: React.FC<FreeHandPositionProps> = ({
  initialMonitors,
  customMonitors,
  ...
  async function createMonitorContainer(monitor: FrontendMonitor): Promise<Container> {
    ...
    switch (monitor.outputs[0].rotation) {
      case Rotation.Inverted:
        ...
        textBackgroundGraphic.rect(
          monitorWidth - monitorText.width,
          monitorHeight - monitorText.height,
          monitorText.width,
          monitorText.height
        );
        break;
      case Rotation.Right:
        ...
        textBackgroundGraphic.rect(
          monitorWidth - monitorText.height,
          monitorHeight - monitorText.width,
          monitorText.height,
          monitorText.width
        );
        break;
  ...
  function convertContainerPoints2MonitorPoints(x: number, y: number): point {
    return {
      x: (x - screenDragOffsetTotal.current.x) * monitorScaleRef.current,
      y: (y - screenDragOffsetTotal.current.y) * monitorScaleRef.current,
    };
  }
  ///Generates 8 points
  function container2Points(monitor: Container): PointAndSource[] {
    ...
    return [
      ...
      {
        monitorName: monitor.label,
        x: middleX,
        y: yScaled,
        pointRelative: PointRelative.TopMiddle,
      },
      { monitorName: monitor.label, x: right, y: yScaled, pointRelative: PointRelative.TopRight },
      {
        ...
        x: xScaled,
        y: middleY,
        pointRelative: PointRelative.MiddleLeft,
      },
      {
        monitorName: monitor.label,
        x: right,
        y: middleY,
        pointRelative: PointRelative.MiddleRight,
      },
      {
        monitorName: monitor.label,
        x: xScaled,
        y: bottom,
        pointRelative: PointRelative.BottomLeft,
      },
      {
        monitorName: monitor.label,
        x: middleX,
        y: bottom,
        pointRelative: PointRelative.BottomMiddle,
      },
  ...
  function points2PointDiff(
    dropPoints: PointAndSource[],
    targetPoints: PointAndSource[]
  ): pointDiff[] {
    ...
    if (dragTarget.current && app.current) {
      if (snapEnabledRef.current) {
        ...
        for (let i = 0; i < customMonitorsRef.current.length; i++) {
          ...
          if (
            focusedMonitor.name == dragTarget.current.label ||
            !focusedMonitor.outputs[0].enabled
          ) {
            continue;
          }
        ...
        let { x: xScaled, y: yScaled } = convertContainerPoints2MonitorPoints(
  ...
  function resetMonitorsPositions() {
    ...
    setCustMonitors(mons =>
      mons.map((curMon, idx) => ({
        ...curMon,
        x: initialMonitors.current[idx].x,
        y: initialMonitors.current[idx].y,
      }))
    );
    screenDragOffsetTotal.current.x = 0;
    screenDragOffsetTotal.current.y = 0;
