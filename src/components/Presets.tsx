...
export const Presets: React.FC<PresetsProps> = ({
    ...
    for (let i = 0; i < customMonitors.length; i++) {
      ...
      let presetAttempt = preset.monitors.find(
        presetMon =>
          ...
          customMonitors[i].outputs[0].modes.find(
            mode => mode.xid === presetMon.outputs[0].currentMode.xid
          )
      );
      if (presetAttempt) {
  ...
  function overwriteFocusedPreset() {
    if (focusedPresetValue && normalizePositionsRef.current) {
      ...
      let newMonitors = normalizedMonitors.map(mon => ({
        ...mon,
        x: Number(mon.x.toFixed(0)),
        y: Number(mon.y.toFixed(0)),
      }));
      ...
      invoke('create_preset', {
        ...
        .then(_res => {
          setPresets(oldPresets =>
            oldPresets.map(preset => (preset.name == focusedPresetValue.name ? newPreset : preset))
          );
  ...
  function deletePreset(presetName: string) {
    ...
    invoke('delete_preset', { presetName })
      .then(_res => {
          ...
          if (presets.findIndex(preset => preset.name == presetSearchTerm) !== -1) {
            setPresets(oldPresets =>
              oldPresets.map(preset =>
                preset.name == presetSearchTerm ? { name: preset.name, monitors: [] } : preset
              )
            );
          } else {
            setPresets(oldPresets => {
    ...
    <div style={{ width: '20vw', height: '100%' }}>
      <div className='presets-top-container'>
        ...
        <div style={{ display: 'flex', flexDirection: 'row' }}>
          <input
            className='presets-search-bar'
            title='Search Preset or Name New Preset'
            type='text'
            value={presetSearchTerm}
            onChange={eve => {
              setPresetSearchTerm(eve.target.value);
            }}
          />
          <button title='Add Preset' className='presets-add-button' onClick={createPreset}>
            +
          </button>
        </div>
      </div>
      ...
      <div className='presets-list-container'>
        {presets
          .filter(preset => preset.name.includes(presetSearchTerm))
          ...
          .map(preset => (
            <div key={preset.name} style={{ display: 'flex', flexDirection: 'row' }}>
              <button
                title='Focus Preset'
                className={
                  focusedPresetValue && focusedPresetValue.name === preset.name
      ...
      <button
        ...
        title='Overwrite Focused Preset With Current Custom Settings'
        onClick={overwriteFocusedPreset}
      >
        Overwrite Preset
      </button>
    </div>
