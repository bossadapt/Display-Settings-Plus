...
import { SingleError } from './components/Popups/SingleErrorPopUp';
function App() {
  ...
  const [singleErrorText, setSingleErrorText] = useState(
    'Failed due to blah blah blahblahblahblahblahblah blah blah blah blah blah blah blah blah'
  );
  ...
  async function refreshMonitors() {
    ...
    invoke<[FrontendMonitor[], String[]]>('get_monitors', {})
      .then(res => {
  ...
  return (
    <div>
      {customMonitorsInfo.length != 0 ? (
        <LoadedScreen
          singleErrorProps={singleErrorProps}
          outputNames={outputNames}
          presets={presets}
          setPresets={setPresets}
          monitorRefreshRef={refreshMonitorsRef}
          customMonitors={customMonitorsInfo}
          initialMonitors={initialMonitorsInfo}
          setCustMonitors={setCustomMonitorsInfo}
        />
      ) : (
        <LoadingScreen />
      )}
